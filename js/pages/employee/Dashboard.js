        ...
        const content = `
            <div class="fade-in" style="max-width: 1120px; margin: 0 auto;">
                <div class="page-header">
                    ...
                    <div class="metric-card" style="background: var(--accent-gradient); color: white; border: none; box-shadow: 0 10px 25px rgba(99, 102, 241, 0.25);">
                        <p style="margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.08em; color: rgba(255,255,255,0.8);">This week</p>
                        <strong style="color: white;">${currentWeekHours}h</strong>
                                    ...
                                    <tbody>
                                        ${timesheets.length > 0 ? timesheets.map(ts => {
                                            ...
                                            return `
                                                <tr>
                                                    <td>
                                                        <div style="font-weight: 700; color: var(--text-main);">Week of ${weekDate}</div>
                                                        <div style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">Submitted ${new Date(ts.submittedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                                                    </td>
                                                    <td><span style="display: inline-flex; align-items: center; gap: 6px; padding: 5px 12px; border-radius: 999px; background: var(--bg-elevated); color: var(--text-main); font-weight: 700; font-size: 13px;">${total}h</span></td>
                                                    <td><span class="badge ${ts.status === 'Approved' ? 'success' : 'warning'}">${ts.status.toUpperCase()}</span></td>
                                                    <td style="text-align: right;"><button class="btn btn-ghost" style="padding: 8px; color: var(--accent-hover);"><i data-lucide="arrow-right-circle" style="width: 20px;"></i></button></td>
                                                </tr>
                                            `;
                                        }).join('') : `
                                            <tr>
                                                <td colspan="4" style="text-align: center; padding: 60px; color: var(--text-light); background: transparent;">
                                                    <div style="margin: 0 auto 18px auto; width: 56px; height: 56px; border-radius: 50%; background: var(--accent-glow); display: grid; place-items: center;">
                                                        <i data-lucide="calendar-x" style="width: 26px; height: 26px; color: var(--accent-hover);"></i>
                                                    </div>
                                                    <div style="font-weight: 700; margin-bottom: 6px; color: var(--text-main);">Good start, but empty.</div>
                                                    <div style="font-size: 13px; color: var(--text-muted);">Submit your first timesheet to see the report.</div>
                                                </td>
                                            </tr>
                                        `}
                                    </tbody>
                    ...
                    <div style="display: grid; gap: 24px;">
                        <div class="form-panel">
                            ...
                            <form id="profile-form">
                                ...
                                <button type="submit" class="btn btn-primary btn-pill" style="margin-top: 24px; width: 100%;">Save profile</button>
                            </form>
                        </div>
                        ...
                        <div class="form-panel">
                            ...
                            <form id="password-form">
                                ...
                                <button type="submit" class="btn btn-outline btn-pill" style="margin-top: 24px; width: 100%;">Reset password</button>
                            </form>
                        </div>

                        <div class="card" style="padding: 20px; border-radius: 20px; background: var(--accent-glow); border: 1px solid rgba(99, 102, 241, 0.2);">
                            <div style="display: flex; gap: 12px; align-items: start;">
                                <i data-lucide="info" style="color: var(--accent-hover); width: 20px; flex-shrink: 0; margin-top: 2px;"></i>
                                <div>
                                    <strong style="display: block; margin-bottom: 8px; color: var(--text-main);">Keep your profile current</strong>
                                    <p style="margin: 0; color: var(--text-muted); font-size: 13px; line-height: 1.6;">Updating your profile ensures project reporting and payouts stay accurate. Your data stays only in browser storage unless shared explicitly.</p>
                                </div>
                            </div>
                        </div>
                    </div>
